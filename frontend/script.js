// CÓDIGO COM DEPURAÇÃO DO MODO AUTOMÁTICO
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
            ...
            if (autoMarkEnabled) {
                ...
                if (lastHandPosition) {
                    ...
                    // LOG DE DEPURAÇÃO DA VELOCIDADE
                    console.log(`Velocidade da mão: ${velocity.toFixed(4)}`);
                    ...
                    if (isHandMoving !== wasHandMoving && movementCooldown <= 0) {
                        if (isHandMoving) {
                            console.log("DETEÇÃO: Mão começou a mover-se! A clicar em INÍCIO.");
                            markStartBtn.click();
                            movementCooldown = 30;
                        } else {
                            console.log("DETEÇÃO: Mão PAROU! A clicar em FIM.");
                            markEndBtn.click();
                            movementCooldown = 30;
                        }
                    }
    ...
    // O resto do código continua igual. Copie e cole tudo para garantir.
    ...
    autoMarkCheckbox.addEventListener('change', (e) => { autoMarkEnabled = e.target.checked; if(autoMarkEnabled) { console.log("Modo de marcação automática ATIVADO."); isHandMoving = false; lastHandPosition = null; } else { console.log("Modo de marcação automática DESATIVADO."); } });
