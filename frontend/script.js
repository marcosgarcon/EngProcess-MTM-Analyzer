...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let startTime = null, elementCounter = 0, autoMarkEnabled = false, lastHandPosition = null, isHandMoving = false, MOVEMENT_THRESHOLD = 0.008, movementCooldown = 0;
    // 3. INICIALIZAÇÃO DA IA
    const hands = new Hands({ locateFile: (file) => `https://cdn.jsdelivr.net/npm/@mediapipe/hands/${file}` });
    hands.setOptions({ maxNumHands: 1, modelComplexity: 1, minDetectionConfidence: 0.7, minTrackingConfidence: 0.7 });
    hands.onResults(onResults);
    // 4. DEFINIÇÃO DE FUNÇÕES
    function exportTableToCSV(filename) { if (analysisTableBody.rows.length === 0) { alert("A tabela está vazia."); return; } let c = ""; const rows = document.querySelectorAll("#analysisTable tr"); rows.forEach(row => { const cols = row.querySelectorAll("td, th"); const rd = []; cols.forEach(col => { let d = col.innerText.replace(/"/g, '""'); rd.push(`"${d}"`); }); c += rd.join(",") + "\r\n"; }); const b = new Blob([c], { type: 'text/csv;charset=utf-8;' }); const l = document.createElement("a"); l.href = URL.createObjectURL(b); l.download = filename; l.style.display = 'none'; document.body.appendChild(l); l.click(); document.body.removeChild(l); }
    function updateSummary() { const r = analysisTableBody.querySelectorAll('tr'); const c = document.querySelectorAll('.standard-time-cell'); let t = 0; c.forEach(cell => { const v = parseFloat(cell.textContent); if (!isNaN(v)) t += v; }); const e = r.length; const a = e > 0 ? t / e : 0; totalStandardTimeEl.textContent = `${t.toFixed(2)} s`; totalElementsEl.textContent = e; averageTimeEl.textContent = `${a.toFixed(2)} s`; };
    ...
    videoUpload.addEventListener('change', (e) => { const f = e.target.files[0]; if (f) { videoPlayer.src = URL.createObjectURL(f); videoPlayer.style.display = 'block'; } });
    ...
    analysisTableBody.addEventListener('input', (e) => { const c = e.target.closest('td'); if (c && c.classList.contains('mtm-code-cell')) { const m = c.innerText.trim().toUpperCase(); const t = c.parentElement.querySelector('.tmu-cell'); if (t) { if (m.length >= 2) { fetchTMU(m, t); } else { t.textContent = ''; } } } });
