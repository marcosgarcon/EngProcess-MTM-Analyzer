...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleMarkEnd() {
        ...
        newRow.innerHTML = `<td>${elementCounter}</td><td>${startTime.toFixed(3)}</td><td>${endTime.toFixed(3)}</td><td>${delta.toFixed(3)}</td><td contenteditable="true"></td><td contenteditable="true"></td><td></td><td class="standard-time-cell">${standardTime.toFixed(4)}</td>`;
    ...
    async function fetchTMU(code, targetCell) {
        try {
            ...
            if (!response.ok) { targetCell.textContent = 'N/A'; return; }
            const data = await response.json();
            targetCell.textContent = data.tmu;
        } catch (error) { targetCell.textContent = 'Erro'; }
    }

    async function handleGenerateReport() {
        const tableData = [];
        const rows = analysisTableBody.querySelectorAll('tr');
        if (rows.length === 0) { alert("A tabela de análise está vazia."); return; }
        ...
        try {
            ...
            if (!response.ok) throw new Error('Falha ao gerar o laudo.');
            const data = await response.json();
            ...
            if (autoMarkEnabled && movementCooldown <= 0) {
                ...
                if (lastHandPosition) {
                    ...
                    if (isHandMoving !== wasHandMoving) {
                        if (isHandMoving) { markStartBtn.click(); } else { markEndBtn.click(); }
                        movementCooldown = 30;
                    }
                }
    ...
    // ===================================================================
    // 5. ASSOCIAÇÃO DE EVENTOS (EVENT LISTENERS)
    // (Esta é a última coisa que o script faz)
    // ===================================================================
    ...
    autoMarkCheckbox.addEventListener('change', (e) => {
        ...
        if (autoMarkEnabled) { isHandMoving = false; lastHandPosition = null; }
    });

    sensitivitySlider.addEventListener('input', (e) => {
        MOVEMENT_THRESHOLD = e.target.value / 10000;
